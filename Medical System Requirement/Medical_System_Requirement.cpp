...
#include<string.h>
 ...
 main()
 {
  ...
  strcpy(m[0].Exp_Date,"24-04-2020");
   ...
   m[1].price=60;
   m[1].quantity=10;
   strcpy(m[1].Mfg_Date,"12-09-2019");
  strcpy(m[1].Exp_Date,"12-09-2020");
   strcpy(m[1].medicneName,"Meftol");
   strcpy(m[1].Company,"ABC");
   strcpy(m[1].info,"Good Medicine for stomach pain");

   do{
   printf("Enter\n1 - Purchase Medicine\n2 - Enter Information about a Medicine\n3 - No of Medicines in Store\n4 - Medicine Information\n5 - Add Medicine\n6 - Delete a Medicine\n7 - Change Quantity of Medicine\n");
   scanf("%d",&choice);
   switch(choice)
   {
    case 1:
   ...
   for(i=0;i<number;i++)
   {
    if(m[i].id==id)
    {
     ...
     printf("Name%s\nPrice=%d\nAvailable Quantity=%d\nCompany=%s\nMfg Date=%s\nExp Date=%s\n",m[i].medicneName,m[i].price,m[i].quantity,m[i].Company,m[i].Mfg_Date,m[i].Exp_Date);
      ...
      {
       printf("Medicine  not Review\n");
      }
      else
      {
       printf("Medicine Review\n");
       printf("Info=%s\n",m[i].info);
      }
